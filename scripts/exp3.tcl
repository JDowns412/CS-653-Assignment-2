...
if { $k == 100 } {
    ...
    #Create links between the nodes with Droptail queue's
    ...
    # $ns_ duplex-link <node1> <node2> <bw> <delay> <qtype> <args>
    #Create links between the nodes with RED queue's
...
switch -exact -- $j {
        ...
        20 {set tcp [new Agent/TCP/Sack1]}
...
#Setup the first TCP connection
switch -exact -- $j {
        10 {set sink [new Agent/TCPSink]}
        20 {set sink [new Agent/TCPSink/Sack1]}
        default {[new Agent/TCPSink]}
}
...
# for experiment 3 I hope that at this point in the sim, the TCP is considered "steady"
$ns at 0.1 "$cbr start"
$ns at 1 "$ftp start"
$ns at 5 "$cbr stop"
$ns at 5 "$ftp stop"
